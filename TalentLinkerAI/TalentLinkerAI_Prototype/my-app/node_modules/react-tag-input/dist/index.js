...
var ClearAllTags = (props) => {
  return /* @__PURE__ */ jsx("button", { className: props.classNames.clearAll, onClick: props.onClick, children: "Clear all" });
};
...
// node_modules/fuzzify/dist/src/utils.js
var MATCHING_SCORE_WEIGHT = 0.5;
var NORMALIZED_DISTANCE_WEIGHT = 0.5;
var levenshteinFullMatrixSearch = (query, target) => {
  const dp = new Array(query.length + 1).fill(0).map(() => new Array(target.length + 1).fill(0));
...
var getMaxLevenshteinDistance = (query, target) => {
...
var getMatchingIndices = (matrix, query, target) => {
  const matches = [];
  let i = query.length;
  let j = target.length;
  while (i > 0 && j > 0) {
...
var calculateScore = (query, target, matches, distance) => {
  ...
  const score = NORMALIZED_DISTANCE_WEIGHT * (1 - normalizedDistance) + MATCHING_SCORE_WEIGHT * matchingScore;
  return score;
};

// node_modules/fuzzify/dist/src/Fuzzy.js
var Fuzzy = class {
  /**
  ...
  search = (query) => {
    ...
    for (let i = 0; i < this.list.length; i++) {
      const matrix = levenshteinFullMatrixSearch(query.toLowerCase(), this.list[i].toLowerCase());
      const matches = getMatchingIndices(matrix, query.toLowerCase(), this.list[i].toLowerCase());
    ...
    result.forEach((res, index) => {
      const obj = { text: res.text, distance: res.distance };
      if (res.score > 0) {
...
// node_modules/fuzzify/dist/src/index.js
var src_default = Fuzzy_default;
...
var fuzzy = new src_default(countries_default, { includeMatches: true, includeScore: true });
console.log(fuzzy.search("apple"));
var ReactTags = (props) => {
  const {
    autofocus,
    autoFocus,
  ...
  const tagsComponent = !readOnly ? /* @__PURE__ */ jsxs3("div", { className: allClassNames.tagInput, children: [
    ...
    clearAll && tags.length > 0 && /* @__PURE__ */ jsx5(ClearAllTags_default, { classNames: allClassNames, onClick: handleClearAll }),
